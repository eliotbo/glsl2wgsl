...
pub fn show_array_spec_wgsl<F>(f: &mut F,  a: &syntax::ArraySpecifier, t: &syntax::TypeSpecifierNonArray)
where
  F: Write,
{
  for dimension in &a.dimensions {
  ...
  F: Write,
{
  match *expr {
    ...
    syntax::Expr::Assignment(ref v, ref op, ref e) => {
      ...
      if do_close_parens { let _ = f.write_str(")"); }
    }
...
pub fn indent<F>(f: &mut F, i: i32) 
  where 
    F: Write,
    {
      (0..i).for_each( |_| {let _ = f.write_str("\t");} );
    }
...
pub fn show_single_declaration_no_type<F>(f: &mut F, d: &syntax::SingleDeclarationNoType, t: &syntax::FullySpecifiedType, i: Indent)
where
  F: Write,
{
  ...
  F: Write,
{
  match *ed {
    ...
    syntax::ExternalDeclaration::FunctionDefinition(ref fd) => show_function_definition(f, fd, i),

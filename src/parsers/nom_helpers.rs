...
pub fn cnst2<'a, T, E>(t: T) -> impl FnMut(Span<'a>) -> Result<(Span<'a>, T), E>
where
    T: 'a + Clone,
{
    move |i| Ok((i, t.clone()))
...
pub fn eol2(i: Span) -> IResult2<()> {
    let (rest, m) = alt((
        eoi2, // this one goes first because it’s very cheap
        value((), line_ending),
    ))(i)?;
    Ok((rest, m))
}
...
// Apply the `f` parser until `g` succeeds. Both parsers consume the input.
pub fn till2<'a, A, B, F, G>(mut f: F, mut g: G) -> impl FnMut(Span<'a>) -> IResult2<'a, ()>
where
...
// A version of many0 that discards the result of the parser, preventing allocating.
pub fn many0_2<'a, A, F>(mut f: F) -> impl FnMut(Span<'a>) -> IResult2<'a, ()>
where
    F: FnMut(Span<'a>) -> IResult2<'a, A>,
{
    move |i| fold_many0(&mut f, || (), |_, _| ())(i)
...
///
/// Discard any leading newline.
pub fn str_till_eol2(i: Span) -> IResult2<Span> {
    let parser = recognize(till2(
        alt((value((), tag("\\\n")), value((), anychar))),
        eol2,
    ));
    map(parser, |ii| {
...
pub fn blank_space2(i: Span) -> IResult2<Span> {
    recognize(many0_2(alt((multispace1, tag("\\\n")))))(i)
}

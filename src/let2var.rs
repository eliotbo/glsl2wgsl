...
use nom::character::complete::{anychar, char, digit1, space0, space1, line_ending, multispace1};
use nom::character::{is_hex_digit, is_oct_digit};
use nom::combinator::{eof, cut, map, not, opt, peek, recognize, value, verify, success};
use nom::error::{ErrorKind, ParseError as _, VerboseError, VerboseErrorKind};
use nom::multi::{fold_many0, many0, many1, separated_list0, many_till};
...
pub fn till_space_or_colon(i: &str) -> ParserResult<String> {
   map(many_till(anychar, peek(alt((tag(" "), tag(":"))) )),
    |(parsed, v)| { 
...
pub fn till_space(i: &str) -> ParserResult<String> {
  map(many_till(anychar, peek(tag(" "))),
   |(parsed, v)| { 
     let mut s = parsed.iter().collect::<String>();
     s.push_str(v);
     s
...
pub fn decl_is_reassigned(i: & str) -> ParserResult<bool> {
    ...
    name.push_str(" = ");
    let z: ParserResult< bool>  = map(peek(is_repeated(&name)), 
      |x|  x )(rest0);
    return z;
}
...
pub fn write_var_or_let(i: &str) -> ParserResult<String> {
  map(
    opt(peek(decl_is_reassigned)),
    |x| if let Some(true) = x {
